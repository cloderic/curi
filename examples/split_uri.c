...
#define PARSE(type) do {						\
	if (curi_parse_ ## type ## _nt(uri, &s, NULL)			\
	    != curi_status_success) {					\
...
static	int	 query_item_str(void *, const char *, size_t, const char *,
		     size_t);
...
static int
scheme(void *data, const char *str, size_t len)
{
	return printf("scheme: %.*s\n", (int)len, str) > 0;
}

static int
userinfo(void *data, const char *str, size_t len)
{
	return printf("user: %.*s\n", (int)len, str) > 0;
}

static int
host(void *data, const char *str, size_t len)
{
	return printf("host: %.*s\n", (int)len, str) > 0;
}

static int
portStr(void *data, const char *str, size_t len)
{
	return printf("port: %.*s\n", (int)len, str) > 0;
}

static int
path(void *data, const char *str, size_t len)
{
	return printf("path: %.*s\n", (int)len, str) > 0;
}

static int
path_segment(void *data, const char *str, size_t len)
{
	static size_t i;
...
static int
query(void *data, const char *str, size_t len)
{
	return printf("query: %.*s\n", (int)len, str) > 0;
}

static int
query_item_null(void *data, const char *str, size_t len)
{
	static size_t i;
...
static int
query_item_double(void *data, const char *str, size_t len, double n)
{
	return printf("query.%.*s: %f\n", (int)len, str, n) > 0;
}

static int
query_item_int(void *data, const char *str, size_t len, long int n)
{
	return printf("query.%.*s: %ld\n", (int)len, str, n) > 0;
}

static int
query_item_str(void *data, const char *str, size_t len,
...
static int
fragment(void *data, const char *str, size_t len)
{
	return printf("fragment: %.*s\n", (int)len, str) > 0;
}

static void
init_curi_settings(curi_settings *s)
{
...
static int
parse_args(int argc, char **argv, int *flag)
{
